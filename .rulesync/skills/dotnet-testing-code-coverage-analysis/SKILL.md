...
1. **Evaluate Test Completeness**: Check if important logic is tested
1. **Assist Refactoring Decisions**: Understand which areas need more attention

1. **Increase Testing Confidence**: Confirm critical paths are validated
...
1. Click "Run Coverage Test"

1. View results: Test Coverage view, editor display, file explorer display
...
1. View → Other Windows → Fine Code Coverage
1. Coverage report automatically displayed

### Method 3: Using VS Code

1. Open Test Explorer

1. Click "Run Coverage Test" icon
1. View coverage results:
...
1. Pass empty array → Test `array.Length == 0`
1. Single element → Doesn't enter loop

1. Max at beginning → Loop doesn't update max
1. Max in middle → Loop updates max
...
1. **Phase 2**: Supplement boundary condition tests (target 70-80%)
1. **Phase 3**: Handle exception scenarios (target 80-85%)

1. **Maintenance Phase**: New features must have tests
...
1. **Exception Testing**: Verify error handling logic
1. **Main Flow Testing**: Cover normal business processes

1. **Condition Branch Testing**: Ensure all branches are tested
...
1. Check if runsettings configuration is correct
1. Confirm tests actually executed

1. Check if exclusion settings are too broad
...
1. Re-run "Run Coverage Test"
1. Check if lcov file is generated

1. Try reloading window
...
1. Add exception scenario tests
1. Consider refactoring overly complex methods
